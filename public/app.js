...
function renderTask(task) {
    ...
    li.querySelector('.toggle-btn').addEventListener('change', () => toggleTask(task.id, li));
    ...
    if (res.ok) {
        ...
        if (data.completed) {
            ...
            if (!element.querySelector('.badge')) {
                element.querySelector('.d-flex').insertAdjacentHTML('beforeend', '<span class="badge bg-success rounded-pill ms-2">Completed</span>');
            }
        } else {
